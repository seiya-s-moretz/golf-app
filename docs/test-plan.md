...
2. **ブロックした相手との会話が一覧に残り続ける**
   - `listConversations`だけが`excludeBlockedUsers`を通っていなかった。送信すると403になる会話がプレビュー付きで残り、閉じる手段が無い（4-10章#4と同じ「死んだ会話」問題の、Connection作成後の経路）
   - 修正: 会話一覧にもブロック除外を適用。なお`GET /conversations/{id}/messages`（履歴閲覧）は制限していない。ブロックは以後の接触を断つ機能であり、既存履歴の閲覧まで遡って遮断する仕様かは事業判断のため（一覧から消えるのでUI上の導線は無い）
3. **不正なIDでサーバーが500を返す**
...
- **`GET /board`が全件返す**: 4-10章の「一覧系APIのページネーション未実装」と同じ論点。投稿が増え続けるため最も早く限界が来る一覧であり、サーバー・クライアント双方の対応が必要
- **`before`カーソルが同時刻のドキュメントを取りこぼす**: `createdAt < cursor`という厳密不等号のため、`createdAt`がミリ秒まで一致するドキュメントがページ境界にあると次ページ以降で読めなくなる。`(createdAt, documentId)`の複合カーソルが必要
- **不正な`before`/`limit`を黙って無視する**: `before=garbage`は1ページ目を返し、`limit=abc`は既定値20になる。ページングし続けるクライアントが無限ループしうるため、400で返すほうが安全
- **`markConversationRead`が会話の全メッセージを読む**: 未読が0件でもペアの全メッセージを取得している。`unreadCount === 0`での早期リターンか、`senderId`を含む複合インデックスで削減できる
