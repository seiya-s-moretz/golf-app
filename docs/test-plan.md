...
2. **ブロックした相手との会話が一覧に残り続ける（仕様変更のうえ対応）**
   - `listConversations`だけが`excludeBlockedUsers`を通っておらず、送信すると403になる会話がプレビュー付きで残る点をレビューで指摘した。当初の技術設計書5-2章は「既に成立している`Connection`・メッセージ履歴は削除しない（ブロック後も履歴閲覧は可能、新規送信のみ拒否）」と明記しており、除外は設計と矛盾するため一度撤回してプロダクトオーナーに確認した
   - **2026-08-13、プロダクトオーナーの決定により「ブロック中は一切表示しない」へ仕様変更**。会話一覧から除外し、`GET /conversations/{partnerId}/messages`（履歴取得）も403とする（一覧から消すだけではAPIを直接叩けば読めてしまうため）。**データ自体は削除しない**ため、ブロック解除で元に戻る。技術設計書5-2章に反映済み
   - あわせて、誤操作で会話が見えなくなることから**ブロック実行時の確認ダイアログを必須**とする決定も受けた。`UserCard`に`AlertDialog`が既にあり要件を満たしていたため、文言に「これまでのやりとりも表示されなくなります（解除すると元に戻ります）」を追記した
   - **ブロック導線の拡充（2026-08-13）**: ブロックできるのが「おすすめユーザー」画面のみで、掲示板やトーク画面で迷惑行為を受けても一覧まで戻らないとブロックできない状態だったため、**掲示板の投稿カード**と**メッセージスレッド画面**にも導線を追加した。確認ダイアログは共通部品`BlockConfirmDialog`に切り出し、3か所で文言・挙動が揃うようにしている（導線ごとに文言がばらつくと、影響範囲の説明が食い違う）。掲示板ではブロック後に該当投稿を一覧から即時除去し、トーク画面ではブロック後に会話自体が取得できなくなるため画面を閉じて一覧へ戻る
3. **不正なIDでサーバーが500を返す**
...
7. **おすすめユーザーの全件走査とN+1**（4-10章、2026-08-13追加対応）
   - **N+1の解消**: ユーザー一覧を返すAPI（おすすめ・会話一覧・ブロック一覧）は、1ユーザーごとに`getAreaById`を呼んでいた。`toUserResponses`（複数ユーザーの一括変換）を追加し、エリアは`getAreasByIds`で1回にまとめて取得する。会話一覧では「つながり数の約3倍」だった読み取りが約2倍に減る
   - **全件走査の解消**: `db.collection("users").get()`で全ユーザーを読んでいた（5,000人規模なら1リクエストあたり5,000件超の読み取り）。配点（スコア差±10:40点／エリア一致:40点／目的一致:20点、閾値60点）から**60点以上には3条件のうち2つ以上が必要**であり、推薦対象は必ず「(A)エリア一致」か「(B)目的一致かつスコア差±10」のどちらかに含まれる。この2本のクエリの和集合を候補とすることで、**結果を変えずに**走査対象を絞った
   - スコアリング自体は従来どおり`calculateRecommendScore`が最終判定する（正解を1か所に保ち、クエリは候補を狭めるだけ）。60点未満の候補が混ざっても結果は変わらない
